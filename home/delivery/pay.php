...
if(isset($_POST['pcode'])){
    ...
    $stmt = $conn->prepare('SELECT product_code FROM delivery WHERE product_code=?');
    ...
    if (!$code) {
	    $query = $conn->prepare('INSERT INTO delivery (image,name,price,quantity,total_price,product_code) VALUES (?,?,?,?,?,?)');
...
if (isset($_GET['clear'])) {
    $stmt = $conn->prepare('DELETE FROM delivery');
    $stmt->execute();
    $_SESSION['showAlert'] = 'block';
...
if (isset($_POST['quantity'])) {
    ...
    $stmt = $conn->prepare('UPDATE delivery SET quantity=?, total_price=? WHERE id=?');
