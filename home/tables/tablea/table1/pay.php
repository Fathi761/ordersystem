...
if(isset($_POST['pcode'])){
    ...
    $stmt = $conn->prepare('SELECT product_code FROM cart1 WHERE product_code=?');
    ...
    if (!$code) {
	    $query = $conn->prepare('INSERT INTO cart1 (image,name,price,quantity,total_price,product_code) VALUES (?,?,?,?,?,?)');
...
if (isset($_POST['quantity'])) {
    ...
    $stmt = $conn->prepare('UPDATE cart1 SET quantity=?, total_price=? WHERE id=?');
